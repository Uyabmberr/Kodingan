...
export default function MemberPage() {
  ...
  useEffect(() => {
    ...
    const cookieAuth = typeof document !== 'undefined' && document.cookie.includes('guppy_auth=true');
    if (savedAuth === 'true' || cookieAuth) setIsAuthenticated(true);
  }, []);
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (inputCode === SECRET_CODE) {
      ...
      // Also set a cookie so server middleware can validate membership
      try {
        document.cookie = `guppy_auth=true; path=/; max-age=${60 * 60 * 24 * 30}; SameSite=Lax`;
      } catch (err) {
        // ignore in SSR
      }
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {categories.map((cat) => (
            <button
              ...
              className={`relative px-6 py-3 rounded-full font-bold text-sm tracking-widest uppercase transition-all ${activeTab === cat.id
                  ...
                  <div className="w-full h-full flex items-center justify-center text-center p-4">
                    {video.youtubeId ? (
                      ...
                      <div className="w-full h-full flex flex-col items-center justify-center">
                        <div className="bg-red-600 text-white px-4 py-2 rounded-full text-sm font-bold mb-4">Video tersedia</div>
                        <p className="text-gray-300">Video akan segera dipublikasikan.</p>
                      </div>
                    )}
                  </div>
                ) : (
